...
export const AccountLinksSettings: React.FC<{ language: 'tr' | 'en' }> = ({ language }) => {
  ...
  const handleGithubLink = async () => {
    ...
    setError(null);
    setNotice(null);
    const result = await linkGithub(username);
    if (!result.ok) setError(result.error || null);
    else {
      setNotice(tr ? 'GitHub hesabınız bağlandı.' : 'GitHub account linked.');
      setGithubInput('');
    }
    setBusy(false);
    await load();
          ...
          <div className="min-w-0 space-y-0.5">
            ...
            <p className="user-text text-[11px] leading-relaxed text-zinc-400">
              {githubLinked
                ...
                : githubRequired
                ? tr
                  ? 'Lanux ile giriş yaptın, bu yüzden hesabında GitHub kimliği yok. Depolarını görebilmek için GitHub kullanıcı adını bağla.'
                  : 'You signed in with Lanux, so this account has no GitHub identity. Link your GitHub username to see your repositories.'
                : tr
                ? 'Depolarını profilinde göstermek için GitHub kullanıcı adını bağla.'
                : 'Link your GitHub username to show your repositories.'}
            </p>
          </div>
        </div>
        ...
        {!githubLinked && (
          <div className="flex flex-col gap-2 sm:flex-row">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') void handleGithubLink();
              }}
              placeholder={tr ? 'GitHub kullanıcı adın' : 'Your GitHub username'}
              aria-label={tr ? 'GitHub kullanıcı adı' : 'GitHub username'}
              className="min-h-11 flex-1 rounded-xl border border-zinc-800 bg-zinc-900 px-3 text-xs text-white placeholder-zinc-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            <button
              type="button"
              onClick={handleGithubLink}
              disabled={busy || githubInput.trim().length === 0}
              className="min-h-11 flex items-center justify-center gap-1.5 rounded-xl bg-zinc-100 px-4 text-xs font-bold text-zinc-950 transition-colors hover:bg-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Link2 className="h-3.5 w-3.5" />}
              <span>{tr ? 'Bağla' : 'Link'}</span>
            </button>
          </div>
        )}
      </div>
