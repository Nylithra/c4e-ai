...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'beta' | 'badges' | 'definitions' | 'platform' | 'database' | 'errors' | 'post_reports'>('beta');
  ...
  const handleAssignSubscription = () => {
    if (!assignUserId || !assignPlanId) {
      alert('Lütfen bir kullanıcı ve destekçi paketi seçiniz.');
      return;
    }
  ...
  const handleSavePlan = () => {
    ...
    showNotification(`Destekçi paketi kaydedildi.`);
  };

  const handleDeletePlan = (planId: string) => {
    if (confirm('Bu destekçi paketini silmek istediğinize emin misiniz?')) {
      const updated = subscriptionPlans.filter((p) => p.id !== planId);
      onSaveSubscriptionPlans(updated);
      showNotification('Destekçi paketi silindi.');
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-400 font-mono">
              Sistem yönetimi, Rozetler, Platform markası ve Destekçi ayarları
            </p>
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold text-white">
              {editingPlan ? 'Destekçi Paketini Düzenle' : 'Yeni Destekçi Paketi Ekle'}
            </h3>
